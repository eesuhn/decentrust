...
contract Company {
  ...
  constructor(address _owner, string memory _companyPublicKey) {
    owner = _owner;
  ...
  function createJobListing(
    ...
    jobCounter++;
    jobListings[jobCounter] = JobListing(jobCounter, _title, _description, true);
  ...
  function submitApplication(
    ...
    applications[applicationCounter] = Application(
      msg.sender, _jobId, _encryptedDataCID, ApplicationStatus.Applied, _gender
    );
  ...
  function giveAssessment(uint256 _applicationId) public onlyOwner {
    ...
    require(
      app.status == ApplicationStatus.Applied, "Invalid application status"
    );
  ...
  function completeAssessment(uint256 _applicationId) public {
    ...
    require(
      app.status == ApplicationStatus.AssessmentGiven, "Assessment not given"
    );
  ...
  function getJobListing(uint256 _jobId)
    public
    view
